...
Solution::Solution(const Abcde& _main_model, const Deep& _aux_model) {
	...
	error = 0.0;
	alpha = 0.0;
	new_posterior.thetha = new Distribution::Thetha[main_model.count_iter];
	new_posterior.w = new double[main_model.count_iter];
}
void Solution::copy_posterior(const Distribution::Posterior &posterior_to, const Distribution::Posterior& posterior_from)
{
...
void Solution::run()
{
	...
	{
		copy_posterior(new_posterior, main_model.posterior);
		for (int i = 0; i < main_model.count_iter; i++)
		{
			if (rand() < 0.05)
			{
		...
		print_log(t);
		copy_posterior(main_model.posterior, new_posterior);
	}
}
